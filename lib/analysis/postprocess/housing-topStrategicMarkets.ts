...
export function injectTopStrategicMarkets(
  ...
  let ranked = candidateFeatures
    .map((feat: any) => {
      ...
      // Enrich: Housing Market Opportunity, Demographics, Tenure Data (check nested first, then lvl1)
      const marketOpportunityRaw = pickFirst(nested, ['market_opportunity', 'housing_opportunity', 'opportunity_score']) ?? pickFirst(lvl1, ['market_opportunity', 'housing_opportunity', 'opportunity_score']);
      const householdsRaw = pickFirst(nested, ['ECYTENHHD', 'total_households', 'households']) ?? pickFirst(lvl1, ['ECYTENHHD', 'total_households', 'households']);
      const populationRaw = pickFirst(nested, ['ECYPTAPOP', 'total_population', 'population']) ?? pickFirst(lvl1, ['ECYPTAPOP', 'total_population', 'population']);
      const incomeRaw = pickFirst(nested, ['ECYHRIMED', 'median_income', 'household_income']) ?? pickFirst(lvl1, ['ECYHRIMED', 'median_income', 'household_income']);
      const homeownershipRaw = pickFirst(nested, ['ECYTENOWN_P', 'homeownership_rate', 'owner_occupied_pct']) ?? pickFirst(lvl1, ['ECYTENOWN_P', 'homeownership_rate', 'owner_occupied_pct']);
      const rentalRaw = pickFirst(nested, ['ECYTENRENT_P', 'rental_rate', 'renter_occupied_pct']) ?? pickFirst(lvl1, ['ECYTENRENT_P', 'rental_rate', 'renter_occupied_pct']);

      const marketOpportunity = marketOpportunityRaw != null ? toNum(marketOpportunityRaw) : null;
      const households = householdsRaw != null ? toNum(householdsRaw) : null;
      const population = populationRaw != null ? toNum(populationRaw) : null;
      const income = incomeRaw != null ? toNum(incomeRaw) : null;
      const homeownership = homeownershipRaw != null ? toNum(homeownershipRaw) : null;
      const rental = rentalRaw != null ? toNum(rentalRaw) : null;
  ...
  if (ranked.length >= 1) {
    const formatted = ranked.map((r: any, i: number) => {
      const bullets = [
        `   • Housing Market Opportunity: ${fmtPct(r.details.marketOpportunity)}`,
        `   • Demographics: Households ${fmtInt(r.details.households)}, Pop ${fmtInt(r.details.population)}, Income ${fmtMoney(r.details.income)}`,
        `   • Housing Tenure: ${fmtPct(r.details.homeownership)} homeowners, ${fmtPct(r.details.rental)} renters`
      ].join('\n');
...
export { injectTopStrategicMarkets as injectTopStrategicHousingMarkets };
