...
export class FeatureImportanceRankingProcessor implements DataProcessorStrategy {
  ...
  validate(rawData: RawAnalysisResult): boolean {
    ...
    const hasRequiredFields = rawData.results.length === 0 || 
      rawData.results.some(record => 
        ...
        (record as any).feature_importance_ranking_score !== undefined
      );
  ...
  process(rawData: RawAnalysisResult): ProcessedAnalysisData {
    ...
    const processedRecords = rawData.results.map((record: any, index: number) => {
      const primaryScore = Number((record as any).feature_importance_ranking_score);
      
      if (isNaN(primaryScore)) {
        throw new Error(`Feature importance ranking record ${(record as any).ID || index} is missing feature_importance_ranking_score`);
      }
