...
export class SpatialClustersProcessor implements DataProcessorStrategy {
  ...
  validate(rawData: RawAnalysisResult): boolean {
    ...
    const hasRequiredFields = rawData.results.length === 0 || 
      rawData.results.some(record => 
        ...
        record.spatial_clusters_score !== undefined
      );
  ...
  process(rawData: RawAnalysisResult): ProcessedAnalysisData {
    ...
    const processedRecords = rawData.results.map((record: any, index: number) => {
      const primaryScore = Number(record.spatial_clusters_score);
      
      if (isNaN(primaryScore)) {
        throw new Error(`Spatial clusters record ${record.ID || index} is missing spatial_clusters_score`);
      }
  ...
  private getTopContributingFields(record: any): Record<string, number> {
    ...
    const fieldDefinitions = [
      { field: 'cluster_density', source: 'cluster_density', importance: 25 },
      { field: 'spatial_autocorrelation', source: 'spatial_autocorrelation', importance: 20 },
      { field: 'neighborhood_similarity', source: 'neighborhood_similarity', importance: 18 },
      { field: 'hotspot_indicator', source: 'hotspot_indicator', importance: 15 },
      { field: 'geographic_concentration', source: 'geographic_concentration', importance: 12 }
    ];
