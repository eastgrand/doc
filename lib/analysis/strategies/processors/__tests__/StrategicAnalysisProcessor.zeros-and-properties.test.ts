...
describe('StrategicAnalysisProcessor regression: zeros and raw properties retention', () => {
      ...
      results: [
        {
          ID: '1',
          DESCRIPTION: 'Area 1',
          strategic_analysis_score: 0, // zero score must be preserved (not coerced)
  ...
  const area2 = out.records.find((r: any) => (r as any).area_id === '2') as any;
    ...
    // Values preserved (not NaN or defaulted)
    expect(typeof area1.value).toBe('number');
    expect(area1.value).toBe(0);
    expect(typeof area1.strategic_analysis_score).toBe('number');
    expect(area1.strategic_analysis_score).toBe(0);
    ...
    // Sanity on second record
    expect(area2.value).toBe(10);
    ...
    if (rendererField) {
      expect(['strategic_analysis_score', (out as any).targetVariable]).toContain(rendererField);
    }
  });
});
