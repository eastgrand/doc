...
*/
  ...
  const handleCreateBuffer = useCallback(async () => {
    ...
    if (bufferType === 'radius') {
      ...
      if (bufferUnit === 'miles') {
        radiusInMeters *= 1609.34;
      } else if (bufferUnit === 'kilometers') {
        radiusInMeters *= 1000;
      }
        ...
        <CardContent className="p-0">
          ...
          {activeStep === 'draw' ? (
            ...
            <div className="space-y-4" data-stepcontent="report">
              <div className="bg-gray-50/50 px-4 py-3 rounded-lg border border-gray-100">
                <span className="text-xs text-gray-600">Choose a report template and generate your infographic</span>
              </div>
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label className="text-xs">Dynamic Infographics</Label>
                  <Button
                ...
                <div className="space-y-2">
                  <Label className="text-xs">Selected Report</Label>
                  <div className="text-sm font-medium text-gray-700">{reports.find((r: Report) => r.id === reportTemplate)?.title || 'No report selected'}</div>
                </div>
                <div className="flex gap-4">
                  <Button
                    variant="outline"
