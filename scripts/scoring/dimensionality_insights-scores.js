 ...
 * Dimensionality Insights Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: dimensionality_insights_score = (0.242 × mp10104a_b_p_normalized) + (0.203 × genalphacy_p_normalized) + (0.191 × mp10116a_b_p_normalized) + (0.183 × mp10120a_b_p_normalized) + (0.181 × x14058_x_a_normalized)
 * 
 * Components (5 total):
...
function calculateDimensionalityInsightsScore(record) {
...
console.log('🔄 Calculating dimensionality_insights scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateDimensionalityInsightsScore(record);
  record.dimensionality_insights_score = score;
...
console.log('📈 Dimensionality Insights Scoring Statistics (Data-Driven):');
...
console.log('📊 Dimensionality Insights Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.dimensionality_insights_score - a.dimensionality_insights_score)
  .slice(0, 15);

console.log('🏆 Top 15 Dimensionality Insights Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.dimensionality_insights_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.dimensionality_insights_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.dimensionality_insights_score
  }))
};
...
console.log(`🎯 All ${processedCount.toLocaleString()} records now include dimensionality_insights_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven dimensionality_insights_score for all records');
