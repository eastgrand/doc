 ...
 * Outlier Detection Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: outlier_detection_score = (0.381 × mp10104a_b_p_normalized) + (0.319 × genalphacy_p_normalized) + (0.300 × mp10116a_b_p_normalized)
 * 
 * Components (3 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/outlier-detection.json');
...
function calculateOutlierDetectionScore(record) {
...
console.log('🔄 Calculating outlier_detection scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateOutlierDetectionScore(record);
...
console.log('📈 Outlier Detection Scoring Statistics (Data-Driven):');
...
console.log('🏆 Top 15 Outlier Detection Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.outlier_detection_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.outlier_detection_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.outlier_detection_score
  }))
};
...
console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven outlier_detection_score for all records');
