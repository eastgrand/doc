 ...
 * Comparative Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: comparative_analysis_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
function calculateComparativeAnalysisScore(record) {
...
console.log('🔄 Calculating comparative_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateComparativeAnalysisScore(record);
...
console.log('📈 Comparative Analysis Scoring Statistics (Data-Driven):');
...
console.log('🏆 Top 15 Comparative Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.comparative_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.comparative_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.comparative_analysis_score
  }))
};
...
console.log(`🎯 All ${processedCount.toLocaleString()} records now include comparative_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven comparative_analysis_score for all records');
