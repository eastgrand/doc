 ...
 * Spatial Clusters Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: spatial_clusters_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
function calculateSpatialClustersScore(record) {
...
console.log('🔄 Calculating spatial_clusters scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateSpatialClustersScore(record);
  record.spatial_clusters_score = score;
...
console.log('📈 Spatial Clusters Scoring Statistics (Data-Driven):');
...
console.log('📊 Spatial Clusters Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.spatial_clusters_score - a.spatial_clusters_score)
  .slice(0, 15);

console.log('🏆 Top 15 Spatial Clusters Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.spatial_clusters_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.spatial_clusters_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.spatial_clusters_score
  }))
};
...
console.log('✅ Spatial Clusters scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include spatial_clusters_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven spatial_clusters_score for all records');
