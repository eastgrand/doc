 ...
 * Segment Profiling Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: segment_profiling_score = (0.201 × mp10104a_b_p_normalized) + (0.191 × genalphacy_p_normalized) + (0.159 × mp10116a_b_p_normalized) + (0.153 × mp10120a_b_p_normalized) + (0.150 × x14058_x_a_normalized) + (0.146 × mp10002a_b_p_normalized)
 * 
 * Components (6 total):
...
function calculateSegmentProfilingScore(record) {
...
console.log('🔄 Calculating segment_profiling scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateSegmentProfilingScore(record);
...
console.log('📈 Segment Profiling Scoring Statistics (Data-Driven):');
...
console.log('🏆 Top 15 Segment Profiling Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.segment_profiling_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.segment_profiling_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.segment_profiling_score
  }))
};
...
console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven segment_profiling_score for all records');
