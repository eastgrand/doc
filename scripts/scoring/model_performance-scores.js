 ...
 * Model Performance Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: model_performance_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/model-performance.json');
...
function calculateModelPerformanceScore(record) {
...
console.log('🔄 Calculating model_performance scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateModelPerformanceScore(record);
  record.model_performance_score = score;
...
console.log('📈 Model Performance Scoring Statistics (Data-Driven):');
...
console.log('📊 Model Performance Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.model_performance_score - a.model_performance_score)
  .slice(0, 15);

console.log('🏆 Top 15 Model Performance Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.model_performance_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.model_performance_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.model_performance_score
  }))
};
...
console.log('✅ Model Performance scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include model_performance_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven model_performance_score for all records');
