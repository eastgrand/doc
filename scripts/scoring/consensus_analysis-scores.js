 ...
 * Consensus Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: consensus_analysis_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
function calculateConsensusAnalysisScore(record) {
...
console.log('🔄 Calculating consensus_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateConsensusAnalysisScore(record);
  record.consensus_analysis_score = score;
...
console.log('📈 Consensus Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Consensus Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.consensus_analysis_score - a.consensus_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Consensus Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.consensus_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.consensus_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.consensus_analysis_score
  }))
};
...
console.log(`🎯 All ${processedCount.toLocaleString()} records now include consensus_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven consensus_analysis_score for all records');
