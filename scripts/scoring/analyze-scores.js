 ...
 * Analyze Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: analyze_score = (0.242 × mp10104a_b_p_normalized) + (0.203 × genalphacy_p_normalized) + (0.191 × mp10116a_b_p_normalized) + (0.183 × mp10120a_b_p_normalized) + (0.181 × x14058_x_a_normalized)
 * 
 * Components (5 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/analyze.json');
...
function calculateAnalyzeScore(record) {
...
console.log('🔄 Calculating analyze scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateAnalyzeScore(record);
  record.analyze_score = score;
...
console.log('📈 Analyze Scoring Statistics (Data-Driven):');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.analyze_score - a.analyze_score)
  .slice(0, 15);

console.log('🏆 Top 15 Analyze Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.analyze_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.analyze_scoring_metadata = {
...
console.log('✅ Analyze scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include analyze_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven analyze_score for all records');
