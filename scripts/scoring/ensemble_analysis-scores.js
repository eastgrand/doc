 ...
 * Ensemble Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: ensemble_analysis_score = (0.242 × mp10104a_b_p_normalized) + (0.203 × genalphacy_p_normalized) + (0.191 × mp10116a_b_p_normalized) + (0.183 × mp10120a_b_p_normalized) + (0.181 × x14058_x_a_normalized)
 * 
 * Components (5 total):
...
function calculateEnsembleAnalysisScore(record) {
...
console.log('🔄 Calculating ensemble_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateEnsembleAnalysisScore(record);
  record.ensemble_analysis_score = score;
...
console.log('📈 Ensemble Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Ensemble Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.ensemble_analysis_score - a.ensemble_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Ensemble Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.ensemble_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.ensemble_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.ensemble_analysis_score
  }))
};
...
console.log(`🎯 All ${processedCount.toLocaleString()} records now include ensemble_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven ensemble_analysis_score for all records');
