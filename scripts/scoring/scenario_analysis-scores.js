 ...
 * Scenario Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: scenario_analysis_score = (0.242 × mp10104a_b_p_normalized) + (0.203 × genalphacy_p_normalized) + (0.191 × mp10116a_b_p_normalized) + (0.183 × mp10120a_b_p_normalized) + (0.181 × x14058_x_a_normalized)
 * 
 * Components (5 total):
...
function calculateScenarioAnalysisScore(record) {
...
console.log('🔄 Calculating scenario_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateScenarioAnalysisScore(record);
...
console.log('📈 Scenario Analysis Scoring Statistics (Data-Driven):');
...
console.log('🏆 Top 15 Scenario Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.scenario_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.scenario_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.scenario_analysis_score
  }))
};
...
console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven scenario_analysis_score for all records');
