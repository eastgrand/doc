 ...
 * Trend Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: trend_analysis_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/trend-analysis.json');
...
function calculateTrendAnalysisScore(record) {
...
console.log('🔄 Calculating trend_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateTrendAnalysisScore(record);
  record.trend_analysis_score = score;
...
console.log('📈 Trend Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Trend Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.trend_analysis_score - a.trend_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Trend Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.trend_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.trend_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.trend_analysis_score
  }))
};
...
console.log('✅ Trend Analysis scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include trend_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven trend_analysis_score for all records');
