 ...
 * Correlation Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: correlation_score = (1.000 × mp10128a_b_p_normalized)
 * 
 * Components (1 total):
 *   1. mp10128a_b_p (weight: 1.000)
 *      Feature contributes to correlation analysis through data-driven importance
 * 
...
function calculateCorrelationScore(record) {
...
console.log('🔄 Calculating correlation scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateCorrelationScore(record);
...
console.log('📈 Correlation Scoring Statistics (Data-Driven):');
...
console.log('📊 Correlation Distribution:');
...
console.log('🏆 Top 15 Correlation Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.correlation_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.correlation_scoring_metadata = {
...
console.log('✅ Correlation scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include correlation_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven correlation_score for all records');
