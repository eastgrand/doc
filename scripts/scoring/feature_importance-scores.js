 ...
 * Feature Importance Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: feature_score = (1.000 × mp10128a_b_p_normalized)
 * 
 * Components (1 total):
 *   1. mp10128a_b_p (weight: 1.000)
 *      Feature contributes to feature_importance analysis through data-driven importance
 * 
...
console.log('🔄 Calculating feature_importance scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateFeatureImportanceScore(record);
  record.feature_importance_score = score;
...
console.log('📈 Feature Importance Scoring Statistics (Data-Driven):');
...
console.log('📊 Feature Importance Distribution:');
...
console.log('🏆 Top 15 Feature Importance Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.feature_importance_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.feature_importance_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.feature_importance_score
  }))
};
...
console.log('✅ Feature Importance scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include feature_importance_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven feature_importance_score for all records');
