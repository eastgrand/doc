 ...
 * Predictive Modeling Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: predictive_modeling_score = (0.242 × mp10104a_b_p_normalized) + (0.203 × genalphacy_p_normalized) + (0.191 × mp10116a_b_p_normalized) + (0.183 × mp10120a_b_p_normalized) + (0.181 × x14058_x_a_normalized)
 * 
 * Components (5 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/predictive-modeling.json');
...
function calculatePredictiveModelingScore(record) {
...
console.log('🔄 Calculating predictive_modeling scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculatePredictiveModelingScore(record);
...
console.log('📈 Predictive Modeling Scoring Statistics (Data-Driven):');
...
console.log('🏆 Top 15 Predictive Modeling Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.predictive_modeling_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.predictive_modeling_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.predictive_modeling_score
  }))
};
...
console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven predictive_modeling_score for all records');
