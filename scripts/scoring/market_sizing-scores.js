 ...
 * Market Sizing Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: market_score = (1.000 × mp10128a_b_p_normalized)
 * 
 * Components (1 total):
 *   1. mp10128a_b_p (weight: 1.000)
 *      Feature contributes to market_sizing analysis through data-driven importance
 * 
...
function calculateMarketSizingScore(record) {
...
console.log('🔄 Calculating market_sizing scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateMarketSizingScore(record);
...
console.log('📈 Market Sizing Scoring Statistics (Data-Driven):');
...
console.log('🏆 Top 15 Market Sizing Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.market_sizing_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.market_sizing_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.market_sizing_score
  }))
};
...
console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven market_sizing_score for all records');
