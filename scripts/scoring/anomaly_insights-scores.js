 ...
 * Anomaly Insights Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: anomaly_insights_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
function calculateAnomalyInsightsScore(record) {
...
console.log('🔄 Calculating anomaly_insights scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateAnomalyInsightsScore(record);
  record.anomaly_insights_score = score;
...
console.log('📈 Anomaly Insights Scoring Statistics (Data-Driven):');
...
console.log('📊 Anomaly Insights Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.anomaly_insights_score - a.anomaly_insights_score)
  .slice(0, 15);

console.log('🏆 Top 15 Anomaly Insights Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.anomaly_insights_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.anomaly_insights_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.anomaly_insights_score
  }))
};
...
console.log(`🎯 All ${processedCount.toLocaleString()} records now include anomaly_insights_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven anomaly_insights_score for all records');
