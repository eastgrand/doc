 ...
 * Feature Interactions Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: feature_interactions_score = (0.242 × mp10104a_b_p_normalized) + (0.203 × genalphacy_p_normalized) + (0.191 × mp10116a_b_p_normalized) + (0.183 × mp10120a_b_p_normalized) + (0.181 × x14058_x_a_normalized)
 * 
 * Components (5 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/feature-interactions.json');
...
function calculateFeatureInteractionsScore(record) {
...
console.log('🔄 Calculating feature_interactions scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateFeatureInteractionsScore(record);
  record.feature_interactions_score = score;
...
console.log('📈 Feature Interactions Scoring Statistics (Data-Driven):');
...
console.log('📊 Feature Interactions Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.feature_interactions_score - a.feature_interactions_score)
  .slice(0, 15);

console.log('🏆 Top 15 Feature Interactions Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.feature_interactions_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.feature_interactions_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.feature_interactions_score
  }))
};
...
console.log('✅ Feature Interactions scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include feature_interactions_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven feature_interactions_score for all records');
