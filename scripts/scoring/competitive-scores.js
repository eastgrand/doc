 ...
 * Competitive Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: competitive_score = (1.000 × mp10128a_b_p_normalized)
 * 
 * Components (1 total):
 *   1. mp10128a_b_p (weight: 1.000)
 *      Feature contributes to competitive analysis through data-driven importance
 * 
...
console.log('🔄 Calculating competitive scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateCompetitiveScore(record);
  record.competitive_score = score;
...
console.log('📈 Competitive Scoring Statistics (Data-Driven):');
...
console.log('📊 Competitive Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.competitive_score - a.competitive_score)
  .slice(0, 15);

console.log('🏆 Top 15 Competitive Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.competitive_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.competitive_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.competitive_score
  }))
};
...
console.log('✅ Competitive scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include competitive_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven competitive_score for all records');
