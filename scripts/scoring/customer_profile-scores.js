 ...
 * Customer Profile Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: customer_profile_score = (0.201 × mp10104a_b_p_normalized) + (0.191 × genalphacy_p_normalized) + (0.159 × mp10116a_b_p_normalized) + (0.153 × mp10120a_b_p_normalized) + (0.150 × x14058_x_a_normalized) + (0.146 × mp10002a_b_p_normalized)
 * 
 * Components (6 total):
 *   1. mp10104a_b_p (weight: 0.201)
 *      Feature contributes to  analysis through data-driven importance
 *   2. genalphacy_p (weight: 0.191)
 *      Feature contributes to  analysis through data-driven importance
 *   3. mp10116a_b_p (weight: 0.159)
 *      Feature contributes to  analysis through data-driven importance
 *   4. mp10120a_b_p (weight: 0.153)
 *      Feature contributes to  analysis through data-driven importance
 *   5. x14058_x_a (weight: 0.150)
 *      Feature contributes to  analysis through data-driven importance
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/customer-profile.json');
...
function calculateCustomerProfileScore(record) {
  ...
  // Component: mp10104a_b_p (weight: 0.201)
  ...
  if (mp10104a_b_p_raw > 0) {
    ...
    totalScore += 0.201 * mp10104a_b_p_normalized;

    componentCount++;
  }

  // Component: genalphacy_p (weight: 0.191)
  ...
  if (genalphacy_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.191 * genalphacy_p_normalized;

    componentCount++;
  }

  // Component: mp10116a_b_p (weight: 0.159)
  ...
  if (mp10116a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.159 * mp10116a_b_p_normalized;

    componentCount++;
  }

  // Component: mp10120a_b_p (weight: 0.153)
  ...
  if (mp10120a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.153 * mp10120a_b_p_normalized;

    componentCount++;
  }

  // Component: x14058_x_a (weight: 0.150)
  ...
  if (x14058_x_a_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.150 * x14058_x_a_normalized;

    componentCount++;
  }

  // Component: mp10002a_b_p (weight: 0.146)
  ...
  if (mp10002a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.146 * mp10002a_b_p_normalized;
...
console.log('🔄 Calculating customer_profile scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateCustomerProfileScore(record);
  record.customer_profile_score = score;
...
console.log('📈 Customer Profile Scoring Statistics (Data-Driven):');
...
console.log('📊 Customer Profile Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.customer_profile_score - a.customer_profile_score)
  .slice(0, 15);

console.log('🏆 Top 15 Customer Profile Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.customer_profile_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.customer_profile_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.customer_profile_score
  }))
};
...
console.log('✅ Customer Profile scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include customer_profile_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven customer_profile_score for all records');
