 ...
 * Correlation Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: correlation_analysis_score = (0.381 × mp10104a_b_p_normalized) + (0.319 × genalphacy_p_normalized) + (0.300 × mp10116a_b_p_normalized)
 * 
 * Components (3 total):
 *   1. mp10104a_b_p (weight: 0.381)
 *      Feature contributes to  analysis through data-driven importance
 *   2. genalphacy_p (weight: 0.319)
 *      Feature contributes to  analysis through data-driven importance
 *   3. mp10116a_b_p (weight: 0.300)
 *      Feature contributes to  analysis through data-driven importance
 * 
...
function calculateCorrelationAnalysisScore(record) {
  ...
  // Component: mp10104a_b_p (weight: 0.381)
  ...
  if (mp10104a_b_p_raw > 0) {
    ...
    totalScore += 0.381 * mp10104a_b_p_normalized;

    componentCount++;
  }

  // Component: genalphacy_p (weight: 0.319)
  ...
  if (genalphacy_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.319 * genalphacy_p_normalized;

    componentCount++;
  }

  // Component: mp10116a_b_p (weight: 0.300)
  ...
  if (mp10116a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.300 * mp10116a_b_p_normalized;
...
console.log('🔄 Calculating correlation_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateCorrelationAnalysisScore(record);
  record.correlation_analysis_score = score;
...
console.log('📈 Correlation Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Correlation Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.correlation_analysis_score - a.correlation_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Correlation Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.correlation_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.correlation_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.correlation_analysis_score
  }))
};
...
console.log('✅ Correlation Analysis scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include correlation_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven correlation_analysis_score for all records');
