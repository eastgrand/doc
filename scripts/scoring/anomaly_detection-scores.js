 ...
 * Anomaly Detection Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: anomaly_detection_score = (0.381 × mp10104a_b_p_normalized) + (0.319 × genalphacy_p_normalized) + (0.300 × mp10116a_b_p_normalized)
 * 
 * Components (3 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/anomaly-detection.json');
...
function calculateAnomalyDetectionScore(record) {
...
console.log('🔄 Calculating anomaly_detection scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateAnomalyDetectionScore(record);
...
console.log('📈 Anomaly Detection Scoring Statistics (Data-Driven):');
...
console.log('🏆 Top 15 Anomaly Detection Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.anomaly_detection_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.anomaly_detection_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.anomaly_detection_score
  }))
};
...
console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven anomaly_detection_score for all records');
