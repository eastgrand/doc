 ...
 * Model Selection Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: model_selection_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
function calculateModelSelectionScore(record) {
...
console.log('🔄 Calculating model_selection scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateModelSelectionScore(record);
  record.model_selection_score = score;
...
console.log('📈 Model Selection Scoring Statistics (Data-Driven):');
...
console.log('📊 Model Selection Distribution:');
...
console.log('🏆 Top 15 Model Selection Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.model_selection_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.model_selection_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.model_selection_score
  }))
};
...
console.log(`🎯 All ${processedCount.toLocaleString()} records now include model_selection_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven model_selection_score for all records');
