 ...
 * Spatial Clustering Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: spatial_score = (1.000 × mp10128a_b_p_normalized)
 * 
 * Components (1 total):
 *   1. mp10128a_b_p (weight: 1.000)
 *      Feature contributes to spatial_clustering analysis through data-driven importance
 * 
...
function calculateSpatialClusteringScore(record) {
...
console.log('🔄 Calculating spatial_clustering scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateSpatialClusteringScore(record);
  record.spatial_clustering_score = score;
...
console.log('📈 Spatial Clustering Scoring Statistics (Data-Driven):');
...
console.log('📊 Spatial Clustering Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.spatial_clustering_score - a.spatial_clustering_score)
  .slice(0, 15);

console.log('🏆 Top 15 Spatial Clustering Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.spatial_clustering_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.spatial_clustering_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.spatial_clustering_score
  }))
};
...
console.log('✅ Spatial Clustering scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include spatial_clustering_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven spatial_clustering_score for all records');
