 ...
 * Brand Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: brand_score = (1.000 × mp10128a_b_p_normalized)
 * 
 * Components (1 total):
 *   1. mp10128a_b_p (weight: 1.000)
 *      Feature contributes to brand_analysis analysis through data-driven importance
 * 
...
function calculateBrandAnalysisScore(record) {
...
console.log('🔄 Calculating brand_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateBrandAnalysisScore(record);
...
console.log('📈 Brand Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Brand Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.brand_analysis_score - a.brand_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Brand Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.brand_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.brand_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.brand_analysis_score
  }))
};
...
console.log('✅ Brand Analysis scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include brand_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven brand_analysis_score for all records');
