 ...
 * Sensitivity Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: sensitivity_analysis_score = (0.295 × mp10104a_b_p_normalized) + (0.248 × genalphacy_p_normalized) + (0.233 × mp10116a_b_p_normalized) + (0.224 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/sensitivity-analysis.json');
...
function calculateSensitivityAnalysisScore(record) {
...
console.log('🔄 Calculating sensitivity_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateSensitivityAnalysisScore(record);
  record.sensitivity_analysis_score = score;
...
console.log('📈 Sensitivity Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Sensitivity Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.sensitivity_analysis_score - a.sensitivity_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Sensitivity Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.sensitivity_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.sensitivity_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.sensitivity_analysis_score
  }))
};
...
console.log('✅ Sensitivity Analysis scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include sensitivity_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven sensitivity_analysis_score for all records');
