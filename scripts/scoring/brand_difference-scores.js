 ...
 * Brand Difference Scoring Script - Data-Driven Algorithm
 * 
 ...
 * 
 * Components (4 total):
 *   1. mp10104a_b_p (weight: 0.370)
 *      Feature contributes to  analysis through data-driven importance
 *   2. genalphacy_p (weight: 0.222)
 *      Feature contributes to  analysis through data-driven importance
 *   3. mp10116a_b_p (weight: 0.208)
 *      Feature contributes to  analysis through data-driven importance
 *   4. mp10120a_b_p (weight: 0.200)
 *      Feature contributes to  analysis through data-driven importance
 * 
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/brand-difference.json');
...
function calculateBrandDifferenceScore(record) {
  ...
  // Component: mp10104a_b_p (weight: 0.370)
  ...
  if (mp10104a_b_p_raw > 0) {
    ...
    totalScore += 0.370 * mp10104a_b_p_normalized;

    componentCount++;
  }

  // Component: genalphacy_p (weight: 0.222)
  ...
  if (genalphacy_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.222 * genalphacy_p_normalized;

    componentCount++;
  }

  // Component: mp10116a_b_p (weight: 0.208)
  ...
  if (mp10116a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.208 * mp10116a_b_p_normalized;

    componentCount++;
  }

  // Component: mp10120a_b_p (weight: 0.200)
  ...
  if (mp10120a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.200 * mp10120a_b_p_normalized;
...
console.log('🔄 Calculating brand_difference scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateBrandDifferenceScore(record);
  record.brand_difference_score = score;
...
console.log('📈 Brand Difference Scoring Statistics (Data-Driven):');
...
console.log('📊 Brand Difference Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.brand_difference_score - a.brand_difference_score)
  .slice(0, 15);

console.log('🏆 Top 15 Brand Difference Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.brand_difference_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.brand_difference_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.brand_difference_score
  }))
};
...
console.log('✅ Brand Difference scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include brand_difference_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven brand_difference_score for all records');
