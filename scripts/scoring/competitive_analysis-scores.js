 ...
 * Competitive Analysis Scoring Script - Data-Driven Algorithm
 * 
 * Generated using data_driven_shap methodology from SHAP feature importance analysis
 * Formula: competitive_analysis_score = (0.301 × mp10104a_b_p_normalized) + (0.238 × mp10116a_b_p_normalized) + (0.233 × genalphacy_p_normalized) + (0.228 × mp10120a_b_p_normalized)
 * 
 * Components (4 total):
 *   1. mp10104a_b_p (weight: 0.301)
 *      Feature contributes to  analysis through data-driven importance
 *   2. mp10116a_b_p (weight: 0.238)
 *      Feature contributes to  analysis through data-driven importance
 *   3. genalphacy_p (weight: 0.233)
 *      Feature contributes to  analysis through data-driven importance
 *   4. mp10120a_b_p (weight: 0.228)
 *      Feature contributes to  analysis through data-driven importance
 * 
...
function calculateCompetitiveAnalysisScore(record) {
  ...
  // Component: mp10104a_b_p (weight: 0.301)
  ...
  if (mp10104a_b_p_raw > 0) {
    ...
    totalScore += 0.301 * mp10104a_b_p_normalized;

    componentCount++;
  }

  // Component: mp10116a_b_p (weight: 0.238)
  ...
  if (mp10116a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.238 * mp10116a_b_p_normalized;

    componentCount++;
  }

  // Component: genalphacy_p (weight: 0.233)
  ...
  if (genalphacy_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.233 * genalphacy_p_normalized;

    componentCount++;
  }

  // Component: mp10120a_b_p (weight: 0.228)
  ...
  if (mp10120a_b_p_raw > 0) {
    ...
    // Transformation: direct normalization
    totalScore += 0.228 * mp10120a_b_p_normalized;
...
console.log('🔄 Calculating competitive_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateCompetitiveAnalysisScore(record);
  record.competitive_analysis_score = score;
...
console.log('📈 Competitive Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Competitive Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.competitive_analysis_score - a.competitive_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Competitive Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.competitive_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.competitive_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.competitive_analysis_score
  }))
};
...
console.log('✅ Competitive Analysis scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include competitive_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven competitive_analysis_score for all records');
