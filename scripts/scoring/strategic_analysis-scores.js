 ...
 * Strategic Analysis Scoring Script - Data-Driven Algorithm
 * 
...
const dataPath = path.join(__dirname, '../../public/data/endpoints/strategic-analysis.json');
...
function calculateStrategicAnalysisScore(record) {
...
console.log('🔄 Calculating strategic_analysis scores using data-driven algorithm...');

analysisData.results.forEach((record, index) => {
  const score = calculateStrategicAnalysisScore(record);
  record.strategic_analysis_score = score;
...
console.log('📈 Strategic Analysis Scoring Statistics (Data-Driven):');
...
console.log('📊 Strategic Analysis Distribution:');
...
const topPerformers = analysisData.results
  .sort((a, b) => b.strategic_analysis_score - a.strategic_analysis_score)
  .slice(0, 15);

console.log('🏆 Top 15 Strategic Analysis Opportunities (Data-Driven):');
topPerformers.forEach((record, index) => {
  console.log(`   ${index + 1}. ${record.DESCRIPTION || record.ID}: ${record.strategic_analysis_score.toFixed(1)} score`);
});

// Add scoring metadata
analysisData.strategic_analysis_scoring_metadata = {
  ...
  top_performers: topPerformers.slice(0, 10).map(record => ({
    ...
    score: record.strategic_analysis_score
  }))
};
...
console.log('✅ Strategic Analysis scoring complete!');
console.log(`📄 Updated dataset saved to: ${dataPath}`);
console.log(`🎯 All ${processedCount.toLocaleString()} records now include strategic_analysis_score field`);

console.log('\n📋 Next steps:');
console.log('   1. ✅ Created data-driven strategic_analysis_score for all records');
