...
export class TrendsCorrelationVisualization extends BaseVisualization<TrendsCorrelationData> {
      ...
      for (const trendsFeature of batch) {
        ...
        for (const demoFeature of demographicFeatures) {
          ...
          try {
            ...
            const intersects = geometryEngine.intersects(projectedCsdGeom as any, projectedFsaGeom as any);
            ...
            if (intersects) {
              ...
              if (typeof value === 'number' && !isNaN(value)) {
                ...
                const intersectionGeom = geometryEngine.intersect(projectedCsdGeom as any, projectedFsaGeom as any);
                
                if (intersectionGeom && intersectionGeom instanceof Polygon) {
                  const intersectionArea = geometryEngine.geodesicArea(intersectionGeom as any, 'square-kilometers');
                  const fsaArea = geometryEngine.geodesicArea(projectedFsaGeom as any, 'square-kilometers');
    ...
    if (joinField && joinField.trim()) {
      ...
      const joinedFeatures = primaryFeatures
        ...
        .map(primary => {
          ...
          if (comparison) {
            return new Graphic({
              geometry: primary.geometry as __esri.Polygon,
              attributes: {
                ...primary.attributes,
    ...
    for (const primary of primaryFeatures) {
      ...
      for (const comparison of comparisonFeatures) {
        ...
        try {
          ...
          const intersects = geometryEngine.intersects(projectedPrimaryGeom as any, projectedComparisonGeom as any);
          
          if (intersects) {
            // Find the intersection area
            const intersectionGeom = geometryEngine.intersect(projectedPrimaryGeom as any, projectedComparisonGeom as any);
            if (intersectionGeom && intersectionGeom instanceof Polygon) {
              const area = geometryEngine.geodesicArea(intersectionGeom as any, 'square-kilometers');
  ...
  private ensureSpatialReference(
    ...
    try {
      ...
      if (geometry1.spatialReference?.isWebMercator) {
        ...
        const projected = projection.project(geometry2, geometry1.spatialReference) as __esri.Polygon;
        return { 
          geom1: geometry1, 
          geom2: projected 
        };
      } else {
        // Project geometry1 to match geometry2's spatial reference
        const projected = projection.project(geometry1, geometry2.spatialReference) as __esri.Polygon;
        return { 
          geom1: projected, 
          geom2: geometry2 
        };
  ...
  async create(
    ...
    try {
      ...
      if (validProjectedFeatures.length > 0 && validProjectedFeatures[0].geometry) {
        const firstGeometry = validProjectedFeatures[0].geometry as __esri.Polygon | null | undefined;
        extent = firstGeometry && firstGeometry.extent ? firstGeometry.extent.clone() : null;
        
        for (let i = 1; i < validProjectedFeatures.length; i++) {
          const geom = validProjectedFeatures[i].geometry;
          const geomExtent = (geom as __esri.Polygon)?.extent;
          if (extent && geomExtent) {
            extent.union(geomExtent as __esri.Extent);
          }
        }
  ...
  public getLegendInfo(): StandardizedLegendData {
    ...
    // Use standardized field mapping for legend title if data is available
    let title = this.layer?.title || 'Correlation';
    if (this.data) {
      const primaryName = FieldMappingHelper.getFriendlyFieldName(this.data.primaryField);
      const comparisonName = FieldMappingHelper.getFriendlyFieldName(this.data.comparisonField);
      title = `${primaryName} vs ${comparisonName}`;
    }
