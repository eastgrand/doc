...
async function runAllTests() {
  ...
  try {
    ...
    // 4. Run Visualization Output Tests
    console.log("\n\n==== TESTING VISUALIZATION OUTPUTS ====\n");
    const { runVisualizationOutputTests } = await import('./visualization-output-test.js');
    await runVisualizationOutputTests();
    
    // 5. Test New Visualization Types
    console.log("\n\n==== TESTING NEW VISUALIZATION TYPES ====\n");
    const { testNewVisualizationTypes } = await import('./visualization-new-types-test.js');
    await testNewVisualizationTypes();
